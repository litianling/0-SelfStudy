...
typedef struct {
    ...
    XForw_back_uio_map maps[ MAX_UIO_MAPS ];
} XForw_back_uio_info;

/***************** Variable Definitions **************************************/
static XForw_back_uio_info uio_info;
...
static int uio_info_read_name(XForw_back_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XForw_back_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XForw_back_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XForw_back_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XForw_back_Initialize(XForw_back *InstancePtr, const char* InstanceName) {
	XForw_back_uio_info *InfoPtr = &uio_info;
